...
class HomePage:
    ...
    def check_body_color(self):
        with allure.step('Check body color hex == #e2e6ee'):
    ...
    def click_category(self, num):
        with allure.step('Click choosen category'):
            self.driver.implicitly_wait(5)
            category = self.driver.find_elements(By.XPATH, '//a[contains(@class, "topbar-button")]')
            category[num].click()
#пофиксить
    def fill_coupon(self):
        with allure.step('Fill coupon random coefs taken in bet lines'):
    ...
    def click_coupon(self):   
        with allure.step('Click on coupon'):
    ...
    def multiplication_coefs_in_coupon(self):
        with allure.step('Multiply all coefs in coupon'):
    ...
    def check_coupon_coef(self):
        with allure.step('Check coupon coef is correct multiplyed'):
...
#пофиксить
    def is_payment_check_correct_visible(self, value):
        with allure.step('Multiply coef to value'):
            ...
            multiply = value * round(float(coef), 2)
            split = str(multiply).split('.')
        with allure.step('Check payment is correct visible'):
            print(multiply, split)
            assert payment.is_displayed() and payment_value[1][:2] == split[0][:2]

    def is_coupon_open(self):
        with allure.step('Check coupon modal open'):
    ...
    def clear_coefs(self):
        with allure.step('Click clear button'):
            ...
            clear_button.click()
        with allure.step('check empty text visible'):
            assert self.driver.find_element(By.XPATH, '//div[@class="ww-coupon-empty__text"]').is_displayed()

    def open_docs(self):
        with allure.step('Scroll to docs and click'):
            self.driver.implicitly_wait(5)
    ...
    def click_doc(self, num):
        with allure.step('Click on choosen document'):
            docs = self.driver.find_elements(By.XPATH, '//a[@class="doc"]')
            docs[num].click()
    
    def download_doc(self):
        with allure.step('Click to download'):
